...
Automotive SPICE (ASPICE) assessment provides automotive software suppliers with a capability
...
evaluated. A 60-question, weighted question bank is derived directly from the ASPICE base
practices and targeted at specific stakeholder roles. A small number of well-targeted questions
is recommended for each session, depending on the stakeholder's role and the session's progress,
instead of asking every question in the bank. A rule-based classifier then scores and ranks
process weaknesses automatically, without needing a labelled training set or a human assessor's
judgement. These weaknesses are made visible on a results dashboard as soon as a session is
completed. The system is implemented end to end with FastAPI, React, and PostgreSQL.

Unlike the prior simulation-based work, this research evaluates the system on real sessions: the
pilot was self-administered, with all seven stakeholder roles represented, and seventeen
sessions started and fifteen completed. The processes the recommendation engine asks about most
often are the same ones the classifier independently scores as the weakest across the pilot, an
indication that the mechanism behaves sensibly on real data, not only on simulated data. A
single session's result can also differ sharply from the overall result for that process,
underscoring the need for early, per-session visibility. The pilot is real, but limited, and the
weights in the question bank have not yet been checked by a certified assessor; both are stated
explicitly as limitations. Overall, the results show that adaptive, continuous ASPICE assessment
is not only a viable idea but a working one.
\\\\
...
need for a continuous assessment of the process in automotive software development, which is
aligned with ASPICE. It then introduces the research questions, contribution of this work, scope
of the work and concludes by providing an overview of the thesis structure.
...
specific points in the project, a certified assessor comes in and evaluates available evidence and
assigns a score to each process on a predetermined scale of NOT achieved to FULLY
achieved~\cite{vdaqmc2023pam,murphy_aspice_0to60}. This method is good for evaluating process
capability at a given time, but does not give much information about what is happening in between
audits.

Here is the heart of the issue of this thesis. After a project has been evaluated, there is no
...
When used as it's intended, it's not really a solution for this: ASPICE doesn't give a signal for
a development team to respond to between audits, it just gives a rating when the audit is
performed.
...
becoming more and more complex and software from one generation to the next must be compatible
with in-vehicle software systems, cloud services, and vehicles~\cite{zhang2021canbus,furst2010challenges}.
...
Automotive SPICE (ASPICE) has established a good framework for Software Development Process
...
could give you a clue to a weakness. An important question here is whether this can be done, using
fewer questions, but more relevant questions?

This idea is further supported by research on reinforcement learning recommender systems, which
...
vary from one ask to another and relate to what has already been addressed, the process areas
...
\begin{itemize}
  \item \textbf{RQ1.} What are the steps needed to convert the base practices of ASPICE SWE.1-6
  into a question bank of auditor-style questions, aligned with SWE processes and stakeholder
  roles, which is structured and weighted?
  \item \textbf{RQ2.} How can a web-based recommendation system narrow down the question bank
  down to a few questions that are well-targeted in the evaluation cycle?
...
The automotive OEMs and suppliers have adopted a process assessment model called automotive
SPICE (ASPICE) to assess the capability of automotive suppliers in software and systems
...
An ASPICE assessment is conducted by a certified assessor that evaluates work products
(documents, models, test records, etc.) and interviews project staff to identify if each base
...
to ask. Dimitrios recognizes the same gap first, which is that once a supplier is certified,
there is no way to directly monitor the supplier's process to ensure that it continues to meet
the requirements, and therefore organizations often use generic project management or
defect-tracking systems, which were not designed to perform process
assessment~\cite{datsogiannis2024early}. This is the particular issue that will be tackled in the
rest of this thesis.
...
rounds~\cite{naeem2020gentleintro,sutton2018reinforcement}. The name is based on a string of slot
machines, that are sometimes referred to as ``one-armed bandits'', and have an unknown and
possibly different payout rate.
...
vector~\cite{li2010contextual,huang2016linucb}. Each arm $a$ has two learned quantities, a
$d \times d$ matrix $A_a$ and a $d$-dimensional vector $b_a$, with $d$ the number of context
features. These quantities are used to estimate a weight vector from,
...
The first term, $\theta_a^\top x$, is the best guess the algorithm has for the reward that it
...
explains the two papers by Dimitrios which this thesis relies on most directly, along with the
...
practice, the process is conducted as a periodic audit where a certified assessor visits a
project at specific intervals, collects evidence of base practices in accordance with the
...
chapter is contrasted. The biggest drawback is pointed out by Dimitrios, which is that the
certification under ASPICE does not offer an inbuilt mechanism to track the compliance of the
process once it has been certified. Consequently, organisations tend to rely on project
management tools or defect tracking systems which were not intended to be used to identify if a
particular base practice is being observed or not~\cite{datsogiannis2024early}. The following
section thus examines the way in which Dimitrios suggests to fill this gap.
...
Dimitrios recommends a system that handles a dynamic questionnaire, instead of a fixed
one~\cite{datsogiannis2024early}. The questions are based on the literature search on automotive
...
\textbf{Relevance to this thesis.} There are two design aspects from this paper that are carried
...
In a similar fashion, Dimitrios extends the concept with a pool of 137 questions and compares
three bandit algorithms directly: $\epsilon$-greedy, LinUCB and Thompson
Sampling~\cite{datsogiannis2025recommender}. As there was no real deployment at that point, the
...
\textbf{Relevance to this thesis.} This is the most similar available comparison of the
...
LinUCB in particular, instead of comparing several algorithms, as it is easier to reproduce and
reason about a single algorithm in a single thesis evaluation, although
Table~\ref{tab:recommender-results} indicates that $\epsilon$-greedy works better in simulation.
This trade-off is again discussed in Chapter~\ref{chap:results}. Thirdly, it tests the system
with actual (non-simulated) pilot sessions, while neither of the previous papers does.
...
driven by the CMAB concept, both conceptually and in a simulation, but have not yet been
implemented in a real setting. This thesis focuses on this space in particular for
SWE.1--SWE.6: it takes the same concept, restricts the scope of the ASPICE, makes a single
choice of algorithm, and tests the system with real sessions instead of simulated sessions. The
way that that system is constructed is explained in Chapter~\ref{chap:architecture}.
...
\begin{itemize}
  ...
  \item \textbf{FR4.} Stakeholders shall only be asked questions that are relevant to their stake
  in the system.
...
If this architecture is mapped onto what was actually developed for this thesis, each box
represents a concrete component. The GUI is built with the React frontend. The Model Controller
is the business logic of the FastAPI backend, specifically the audit session service that carries
...
Figure~\ref{fig:component-responsibilities} shows the same architecture once more, but from a
different perspective: responsibilities. It does not indicate the flow of requests through the
system, but rather what each component is responsible for.
...
(practice effectively absent). Table~\ref{tab:example-question} contains the full set of options
for \texttt{SWE1\_L1\_01}. This weight is what gets translated into a reward once an option is
chosen by the CMAB engine, and the same value is later used as a weakness value when calculating
the process score (Sections~\ref{sec:cmab-engine} and~\ref{sec:weakness-classification}).
...
Before scoring begins, the engine narrows the active question bank down to admissible arms: the
question's stakeholder-role list must include the current user's role, and the question must not
already be in the list of questions asked during the current session. Both are hard constraints,
...
against the context vector built for the current request. The exploration coefficient $\alpha$
...
between $0.0$ and $1.0$ the same way as the reward in Section~\ref{sec:cmab-engine}. Because the
seeded question bank's weights actually run $0, 2, 3, 4, 5$ rather than stopping at $4$, a
handful of individual answers normalize to just above $1.0$. This is not a problem downstream,
since the classification step in Section~\ref{sec:classification-thresholds} treats any value
past its highest threshold the same way regardless -- but it is worth stating clearly rather
...
Three reasons, all visible in that table, drove the final choice. First, \textbf{determinism}.
...
All the system is deployed in Docker containers, both in development and production. That means,
if it works on one machine, it works on any other. There is no ``it works on my machine'' disconnect
between whoever is developing and Anything that is eventually deployed. This is done locally
...
\begin{figure}[h]
  ...
  \caption{Technologies used across the stack.}
...
\begin{figure}[h]
  ...
  \caption{Frontend development stack and data flow.}
...
\hline
\textbf{Dims} & \textbf{Content} & \textbf{How computed} \\
\hline
...
Chapter~\ref{chap:results}.\footnote{I use AI tool in this Chapter as a verification aid to compare the
...
implemented functionality, and to avoid claiming behaviour that was not implemented.}
...
\texttt{TestBuildContext.test\_one\_hot\_role\_encoding}, which checks the actual array
...
\begin{tabular}{|l|c|l|}
\hline
\textbf{Test file} & \textbf{Tests} & \textbf{Outcome} \\
\hline
...
Chapter~\ref{chap:conclusion}. It does offer, however, something the single-account walkthrough
in Section~\ref{sec:functional-verification} could not: real usage data, broken out by role,
which is exactly what the CMAB engine's context vector is geared to respond to.
...
Again, the numbers above are trustworthy only because of the quality-assurance and
functional-verification work earlier in this chapter: the engine was unit-tested against known
inputs, and was observed completing a real session end to end in
Section~\ref{sec:functional-verification}, rather than being taken on faith.

\section{Threats to Validity}

Four threats can be named explicitly, rather than as a generic checklist.

The first is internal validity: the same person built this system and then answered as each of
its seven stakeholder roles. A self-administered pilot carries a real risk of confirmation bias
...
The second concerns what the sessions were actually assessing. ASPICE questions are meaningful
against a specific, real piece of software -- a real requirements document, a real architecture,
real test records. The pilot's answers may have been given against one consistent real project,
or answered more generically, so that the resulting scores reflect not necessarily that
project's actual process maturity, but something closer to the author's own perception of what
each role typically struggles with. This thesis does not draw a sharp line between the two, and
a reader should weigh the pilot's findings accordingly.
...
reported from those same sessions. Taken together, the evidence gathered here answers each of
the four research questions with a working result rather than a proposal for one: a real
question bank actually seeded and served, a recommendation engine whose selection pattern agrees
with an independently-computed weakness score, a classifier that produced a result from real
answers to real questions, and a dashboard that displayed that result to a real stakeholder the
moment a real session finished.
...
open, and where the system goes from here.
...
processes. Extending the question bank to these groups would let the same system give a fuller
picture of a project's ASPICE capability than a software engineering slice alone.
...
Full list of the 60 questions and 300 options used in the seeded question bank, covering SWE.1--SWE.6.

\section*{Legend}
\begin{description}
\item[Weights] 0 = Best / lowest risk; 5 = Worst / highest risk (highest CMAB reward).
\item[Stakeholders] SD = Software Developer; SA = Software Architect; PM = Project Manager; QA = QA Engineer; TE = Test Engineer; TL = Team Lead; ASR = ASPICE Assessor.
\item[Levels] L1 = Performed; L2 = Managed; L3 = Established.
\end{description}

\section*{SWE.1 --- Software Requirements Analysis}

\subsection*{SWE1\_L1\_01}
\begin{description}
\item[ID] SWE1\_L1\_01
\item[Level] L1
\item[Criteria] SWE.1.BP1 --- Specify SW requirements (verifiability, unambiguity)
\item[Identifies] Weak requirement quality, ambiguity, untestable SW requirements
\item[Stakeholder] SA, SD, QA
\item[Question] How are software requirements specified for the current release?
\item[Options] ~\\
...
\item[Weights] 0, 2, 3, 4, 5
\end{description}

\subsection*{SWE1\_L1\_02}
\begin{description}
\item[ID] SWE1\_L1\_02
\item[Level] L1
\item[Criteria] SWE.1.BP2 --- Structure SW requirements
\item[Identifies] Missing prioritisation / categorisation, scope-creep risk
\item[Stakeholder] PM, SA
\item[Question] How are SW requirements categorised and prioritised?
\item[Options] ~\\
...
\item[Weights] 0, 2, 2, 4, 5
\end{description}

\subsection*{SWE1\_L1\_03}
\begin{description}
\item[ID] SWE1\_L1\_03
\item[Level] L1
\item[Criteria] SWE.1.BP3 --- Analyse SW requirements (correctness, technical feasibility)
\item[Identifies] Latent feasibility risk, late re-work
\item[Stakeholder] SA, SD
\item[Question] How is the technical feasibility of new SW requirements analysed before they are baselined?
\item[Options] ~\\
...
\item[Weights] 0, 2, 4, 5, 5
\end{description}

\subsection*{SWE1\_L1\_04}
\begin{description}
\item[ID] SWE1\_L1\_04
\item[Level] L1
\item[Criteria] SWE.1.BP4 --- Analyse impact on operating environment
\item[Identifies] Missed timing / ODD / resource impact
\item[Stakeholder] SA, SD
\item[Question] When SW requirements affect timing, memory, or operating-environment behaviour, how is the impact captured?
\item[Options] ~\\
...
\subsection*{SWE1\_L1\_05}
\begin{description}
\item[ID] SWE1\_L1\_05
\item[Level] L1
\item[Criteria] SWE.1.BP5 --- Ensure consistency \& bidirectional traceability (SYS-req $\leftrightarrow$ SW-req $\leftrightarrow$ SYS-arch)
\item[Identifies] Broken bidirectional traceability --- top assessor finding
\item[Stakeholder] SA, QA, ASR
\item[Question] What is the current state of bidirectional traceability between system requirements and software requirements?
\item[Options] ~\\
A) Tool-enforced bidirectional links, audited every release\\
B) Bidirectional links exist but coverage is partial\\
C) Only forward links (SYS$\rightarrow$SW) exist\\
D) Traceability is maintained in spreadsheets\\
E) No traceability data
\item[Weights] 0, 2, 3, 4, 5
\end{description}

\subsection*{SWE1\_L1\_06}
\begin{description}
\item[ID] SWE1\_L1\_06
\item[Level] L1
\item[Criteria] SWE.1.BP6 --- Communicate agreed SW requirements \& impact
\item[Identifies] Lack of agreement $\rightarrow$ late churn
\item[Stakeholder] PM, SA, TL
\item[Question] How is agreement on the SW requirements baseline reached and recorded?
\item[Options] ~\\
...
\subsection*{SWE1\_L1\_08}
\begin{description}
\item[ID] SWE1\_L1\_08
\item[Level] L1
\item[Criteria] SWE.1.BP5 --- Ensure consistency \& bidirectional traceability (SW-req $\leftrightarrow$ system architecture)
\item[Identifies] Missing allocation of SW requirements to system architecture elements
\item[Stakeholder] SA, QA, ASR
\item[Question] How is bidirectional traceability maintained between software requirements and the system architecture (i.e., allocation of SW requirements to system architecture elements)?
\item[Options] ~\\
...
\subsection*{SWE1\_L1\_09}
\begin{description}
\item[ID] SWE1\_L1\_09
\item[Level] L1
\item[Criteria] SWE.1.BP3 --- Analyse SW requirements (interdependencies)
\item[Identifies] Conflicting or circular requirement dependencies undetected until late
\item[Stakeholder] SA, SD, QA
\item[Question] How are dependencies and potential conflicts between software requirements identified and managed?
\item[Options] ~\\
...
\subsection*{SWE1\_L2\_07}
\begin{description}
\item[ID] SWE1\_L2\_07
\item[Level] L2
\item[Criteria] GP 2.1.1, GP 2.1.2 --- Performance objectives \& planning for SWE.1
\item[Identifies] Missing/weak strategy for the requirements process
\item[Stakeholder] PM, QA
\item[Question] How is the SWE.1 process planned and monitored at project level?
\item[Options] ~\\
...
\subsection*{SWE1\_L2\_10}
\begin{description}
\item[ID] SWE1\_L2\_10
\item[Level] L2
\item[Criteria] GP 2.1.6 --- Manage interfaces between involved parties (SWE.1)
\item[Identifies] Poor coordination between requirements engineering and system engineering / other stakeholders
\item[Stakeholder] SA, PM, TL
\item[Question] How are interfaces between software requirements engineers and other involved parties (system engineering, safety, customer) coordinated for requirement changes?
\item[Options] ~\\
...
\subsection*{SWE1\_L2\_11}
\begin{description}
\item[ID] SWE1\_L2\_11
\item[Level] L2
\item[Criteria] GP 2.2.1--2.2.3 --- Work-product requirements, storage and control (SW requirements)
\item[Identifies] Requirements work products not under configuration/status control
\item[Stakeholder] QA, ASR
\item[Question] How are the software requirement work products stored, versioned, and status-controlled (e.g., Under Work / Reviewed / Released)?
\item[Options] ~\\
...
\subsection*{SWE1\_L3\_12}
\begin{description}
\item[ID] SWE1\_L3\_12
\item[Level] L3
\item[Criteria] GP 3.1.1, GP 3.1.2 --- Standard process \& required competencies (SWE.1)
\item[Identifies] No organisation-wide standard for requirements-engineering methodology/roles
\item[Stakeholder] ASR, PM, SA
\item[Question] Does the organisation maintain a standard requirements-engineering process (templates, tailoring guideline) and a defined role/competency description for the requirements-engineer role?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_01}
\begin{description}
\item[ID] SWE2\_L1\_01
\item[Level] L1
\item[Criteria] SWE.2.BP1 --- Specify static aspects of architecture
\item[Identifies] Missing decomposition / structural views
\item[Stakeholder] SA
\item[Question] How is the static structure of the SW architecture documented?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_02}
\begin{description}
\item[ID] SWE2\_L1\_02
\item[Level] L1
\item[Criteria] SWE.2.BP2 --- Specify dynamic aspects (timing, sequences, state)
\item[Identifies] Hidden timing/concurrency defects
\item[Stakeholder] SA, SD
\item[Question] How are dynamic aspects (timing, sequencing, state) specified for the architecture?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_03}
\begin{description}
\item[ID] SWE2\_L1\_03
\item[Level] L1
\item[Criteria] SWE.2.BP3 --- Analyse architecture, justify chosen design (PAM 4.0 replaces former ``evaluate alternatives'' BP)
\item[Identifies] Unjustified architecture, hidden trade-offs
\item[Stakeholder] SA, ASR
\item[Question] How is the chosen architecture justified against alternatives and quality criteria (modularity, reliability, security)?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_04}
\begin{description}
\item[ID] SWE2\_L1\_04
\item[Level] L1
\item[Criteria] SWE.2.BP3 --- interfaces \& resource consumption
\item[Identifies] Interface ambiguity, resource overruns at HIL/qualification
\item[Stakeholder] SA, SD
\item[Question] How are interfaces and resource-consumption objectives (RAM/ROM/CPU/bandwidth) defined for SW components?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_05}
\begin{description}
\item[ID] SWE2\_L1\_05
\item[Level] L1
\item[Criteria] SWE.2.BP4 --- Consistency \& bidirectional traceability (SW-req $\leftrightarrow$ SW-arch)
\item[Identifies] Architecture/requirement drift
\item[Stakeholder] SA, QA, ASR
\item[Question] How is bidirectional traceability maintained between SW requirements and architectural elements?
\item[Options] ~\\
...
\subsection*{SWE2\_L1\_06}
\begin{description}
\item[ID] SWE2\_L1\_06
\item[Level] L1
\item[Criteria] SWE.2.BP5 --- Communicate agreed architecture
\item[Identifies] Architecture not internalised by team
\item[Stakeholder] SA, TL
\item[Question] How is the agreed architecture communicated and made available to development and test teams?
\item[Options] ~\\
...
\subsection*{SWE2\_L2\_07}
\begin{description}
\item[ID] SWE2\_L2\_07
\item[Level] L2
\item[Criteria] GP 2.2.1, GP 2.2.4 --- Work-product requirements \& review for architecture artefacts
\item[Identifies] Architecture artefacts not reviewed against criteria
\item[Stakeholder] QA, ASR
\item[Question] How are architecture work products (model, ADRs, interface specs) reviewed and approved?
\item[Options] ~\\
...
\subsection*{SWE2\_L2\_08}
\begin{description}
\item[ID] SWE2\_L2\_08
\item[Level] L2
\item[Criteria] GP 2.1.3, GP 2.1.4 --- Resource needs \& qualification (SWE.2)
\item[Identifies] Architects not trained/qualified on modelling method or tool; unavailable licences
\item[Stakeholder] SA, TL, PM
\item[Question] How is the competency of software architects (modelling method/tool skills) and the availability of architecture tooling (e.g., EA/Cameo licences) ensured?
\item[Options] ~\\
...
\subsection*{SWE2\_L2\_09}
\begin{description}
\item[ID] SWE2\_L2\_09
\item[Level] L2
\item[Criteria] GP 2.1.6 --- Manage interfaces between involved parties (SWE.2)
\item[Identifies] Architecture changes not propagated to detailed-design / integration teams
\item[Stakeholder] SA, TL, PM
\item[Question] When the software architecture changes after initial communication, how are affected detailed-design and integration teams notified and re-aligned?
\item[Options] ~\\
...
\subsection*{SWE2\_L2\_11}
\begin{description}
\item[ID] SWE2\_L2\_11
\item[Level] L2
\item[Criteria] GP 2.1.1, GP 2.1.2 --- Strategy \& planning for SWE.2
\item[Identifies] Architecture design activity not planned against objectives/milestones
\item[Stakeholder] PM, SA
\item[Question] How is the SWE.2 architecture design activity planned and monitored at project level (objectives, milestones, schedule)?
\item[Options] ~\\
...
\subsection*{SWE2\_L3\_10}
\begin{description}
\item[ID] SWE2\_L3\_10
\item[Level] L3
\item[Criteria] GP 3.1.2, GP 3.1.3 --- Required competencies \& resources for the standard process (SWE.2)
\item[Identifies] No organisation-level role/competency definition for the architecture role
\item[Stakeholder] ASR, PM, SA
\item[Question] Does the organisation maintain a defined role description (required competencies, responsibilities) and standard tooling/infrastructure for the software architect role?
\item[Options] ~\\
...
\subsection*{SWE3\_L1\_01}
\begin{description}
\item[ID] SWE3\_L1\_01
\item[Level] L1
\item[Criteria] SWE.3.BP1 --- Specify static aspects of detailed design
\item[Identifies] Skipping detailed design, ``code-is-design'' antipattern
\item[Stakeholder] SA, SD
\item[Question] How is the static detailed design of software units captured?
\item[Options] ~\\
...
\subsection*{SWE3\_L1\_02}
\begin{description}
\item[ID] SWE3\_L1\_02
\item[Level] L1
\item[Criteria] SWE.3.BP2 --- Specify dynamic aspects of detailed design
\item[Identifies] Hidden control-flow / state defects
\item[Stakeholder] SD, SA
\item[Question] How are state machines, control flows, and unit interactions described?
\item[Options] ~\\
...
\subsection*{SWE3\_L1\_03}
\begin{description}
\item[ID] SWE3\_L1\_03
\item[Level] L1
\item[Criteria] SWE.3.BP3 --- Develop software units (coding standard adherence)
\item[Identifies] Coding-standard violations, MISRA gaps
\item[Stakeholder] SD, QA
\item[Question] How are coding standards (e.g., MISRA C/C++, AUTOSAR C++14) enforced during unit construction?
\item[Options] ~\\
...
\subsection*{SWE3\_L1\_04}
\begin{description}
\item[ID] SWE3\_L1\_04
\item[Level] L1
\item[Criteria] SWE.3.BP4 --- Consistency \& bidirectional traceability (SW-req $\leftrightarrow$ arch $\leftrightarrow$ detailed design $\leftrightarrow$ units)
\item[Identifies] Multi-hop traceability gap
\item[Stakeholder] SA, QA, ASR
\item[Question] What is the state of traceability from architecture $\rightarrow$ detailed design $\rightarrow$ source units?
\item[Options] ~\\
...
\subsection*{SWE3\_L1\_05}
\begin{description}
\item[ID] SWE3\_L1\_05
\item[Level] L1
\item[Criteria] SWE.3.BP5 --- Communicate agreed detailed design and units
\item[Identifies] Knowledge silo, single-point-of-failure
\item[Stakeholder] SD, TL
\item[Question] How are agreed detailed design and developed units made available to peers and testers?
\item[Options] ~\\
...
\subsection*{SWE3\_L2\_06}
\begin{description}
\item[ID] SWE3\_L2\_06
\item[Level] L2
\item[Criteria] GP 2.1.5 --- Monitor \& adjust process (defect/rework rates per unit)
\item[Identifies] No control loop for code quality
\item[Stakeholder] TL, QA, PM
\item[Question] How is the SWE.3 process monitored at project level (e.g., rework, defect density per unit)?
\item[Options] ~\\
...
\subsection*{SWE3\_L2\_08}
\begin{description}
\item[ID] SWE3\_L2\_08
\item[Level] L2
\item[Criteria] GP 2.2.2, GP 2.2.3 --- Storage and control of work products (source code \& detailed design)
\item[Identifies] Source code / detailed design not under proper configuration control
\item[Stakeholder] QA, SD, ASR
\item[Question] How are source code and detailed-design work products identified, versioned, and controlled during development?
\item[Options] ~\\
...
\subsection*{SWE3\_L2\_09}
\begin{description}
\item[ID] SWE3\_L2\_09
\item[Level] L2
\item[Criteria] GP 2.1.1, GP 2.1.2 --- Strategy \& planning for SWE.3
\item[Identifies] Detailed design \& unit construction activity not planned against objectives/staffing
\item[Stakeholder] PM, TL, SD
\item[Question] How is the SWE.3 detailed design and unit construction activity planned (staffing, schedule, objectives per component)?
\item[Options] ~\\
...
\subsection*{SWE3\_L3\_07}
\begin{description}
\item[ID] SWE3\_L3\_07
\item[Level] L3
\item[Criteria] GP 3.1.1, GP 3.1.4 --- Standard process \& tailoring guidelines
\item[Identifies] Project-specific drift from organisational standard
\item[Stakeholder] ASR, QA, PM
\item[Question] How is the organisation's standard SWE.3 process tailored for this project?
\item[Options] ~\\
...
\subsection*{SWE4\_L1\_01}
\begin{description}
\item[ID] SWE4\_L1\_01
\item[Level] L1
\item[Criteria] SWE.4.BP1 --- Specify unit-verification measures (tactic)
\item[Identifies] Missing verification tactic
\item[Stakeholder] TE, SD
\item[Question] How are unit-verification measures (test cases, static analysis, coverage goals) specified?
\item[Options] ~\\
...
\subsection*{SWE4\_L1\_02}
\begin{description}
\item[ID] SWE4\_L1\_02
\item[Level] L1
\item[Criteria] SWE.4.BP2 --- Select verification measures incl. regression
\item[Identifies] Insufficient regression coverage
\item[Stakeholder] TE
\item[Question] How are verification measures selected per release, including regression?
\item[Options] ~\\
...
\subsection*{SWE4\_L1\_03}
\begin{description}
\item[ID] SWE4\_L1\_03
\item[Level] L1
\item[Criteria] SWE.4.BP3 --- Verify (static + dynamic)
\item[Identifies] Weak static analysis
\item[Stakeholder] SD, QA
\item[Question] How is static analysis performed on software units?
\item[Options] ~\\
...
\subsection*{SWE4\_L1\_04}
\begin{description}
\item[ID] SWE4\_L1\_04
\item[Level] L1
\item[Criteria] SWE.4.BP3 --- Test SW units \& record results (coverage)
\item[Identifies] Inadequate structural coverage
\item[Stakeholder] TE, SD
\item[Question] What unit-test coverage criteria are applied for non-safety code?
\item[Options] ~\\
A) Statement + branch coverage with documented threshold (e.g., $\geq$ 90\% branch)\\
...
\subsection*{SWE4\_L1\_05}
\begin{description}
\item[ID] SWE4\_L1\_05
\item[Level] L1
\item[Criteria] SWE.4.BP4 --- Bidirectional traceability between units, criteria, results
\item[Identifies] Test-result orphaning
\item[Stakeholder] TE, QA, ASR
\item[Question] How are unit-test results linked to verification criteria and units?
\item[Options] ~\\
...
\subsection*{SWE4\_L1\_08}
\begin{description}
\item[ID] SWE4\_L1\_08
\item[Level] L1
\item[Criteria] SWE.4.BP5 --- Summarize and communicate results
\item[Identifies] Unit-verification results not summarised/communicated to affected parties
\item[Stakeholder] SD, TE, TL
\item[Question] How are software unit-verification results (pass/fail, coverage, static-analysis findings) summarised and communicated to affected parties?
\item[Options] ~\\
...
\subsection*{SWE4\_L2\_06}
\begin{description}
\item[ID] SWE4\_L2\_06
\item[Level] L2
\item[Criteria] GP 2.1.6, GP 2.1.7 --- Resources \& stakeholder management for SWE.4
\item[Identifies] Under-resourced verification, late access to HW/test bench
\item[Stakeholder] PM, TL, TE
\item[Question] How is access to test infrastructure (HIL/SIL, test benches, target boards) planned and assured for unit verification?
\item[Options] ~\\
...
\subsection*{SWE4\_L2\_07}
\begin{description}
\item[ID] SWE4\_L2\_07
\item[Level] L2
\item[Criteria] GP 2.2.3, GP 2.2.4 --- Work-product control \& review of unit-verification reports
\item[Identifies] Reports not under configuration control
\item[Stakeholder] QA
\item[Question] How are unit-verification reports stored, versioned, and reviewed?
\item[Options] ~\\
...
\subsection*{SWE4\_L3\_09}
\begin{description}
\item[ID] SWE4\_L3\_09
\item[Level] L3
\item[Criteria] GP 3.1.1, GP 3.1.4 --- Standard process \& monitoring (SWE.4)
\item[Identifies] Unit-verification approach/tooling not standardised or monitored across projects
\item[Stakeholder] ASR, QA, PM
\item[Question] Is there an organisation-wide standard for unit-verification methods/tooling (static-analysis rulesets, coverage thresholds), and is its effectiveness monitored?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_01}
\begin{description}
\item[ID] SWE5\_L1\_01
\item[Level] L1
\item[Criteria] SWE.5.BP1 --- Specify integration-verification measures
\item[Identifies] Missing integration-verification scope
\item[Stakeholder] TE, SA
\item[Question] How are integration-verification measures defined for the integrated software?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_02}
\begin{description}
\item[ID] SWE5\_L1\_02
\item[Level] L1
\item[Criteria] SWE.5.BP2 --- Specify component-behaviour verification measures (PAM 4.0 novelty)
\item[Identifies] Component-level black-box gaps
\item[Stakeholder] TE, SA
\item[Question] How is software-component behaviour verified before integration?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_03}
\begin{description}
\item[ID] SWE5\_L1\_03
\item[Level] L1
\item[Criteria] SWE.5.BP3 --- Select verification measures incl. regression
\item[Identifies] Selection criteria absent
\item[Stakeholder] TE
\item[Question] How are integration-test cases selected per release (including regression)?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_04}
\begin{description}
\item[ID] SWE5\_L1\_04
\item[Level] L1
\item[Criteria] SWE.5.BP4 --- Integrate elements \& perform integration verification
\item[Identifies] Big-bang integration risk
\item[Stakeholder] SD, TE, SA
\item[Question] How are software elements integrated and verified?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_05}
\begin{description}
\item[ID] SWE5\_L1\_05
\item[Level] L1
\item[Criteria] SWE.5.BP5 --- Perform component verification \& record
\item[Identifies] Result records missing
\item[Stakeholder] QA, TE
\item[Question] How are component-verification and integration-verification results recorded?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_06}
\begin{description}
\item[ID] SWE5\_L1\_06
\item[Level] L1
\item[Criteria] SWE.5.BP6 --- Consistency \& bidirectional traceability (arch / detailed design $\leftrightarrow$ verification measures $\leftrightarrow$ results)
\item[Identifies] Verification-result orphaning at integration
\item[Stakeholder] QA, ASR
\item[Question] What is the state of bidirectional traceability between architecture/design, integration verification measures, and results?
\item[Options] ~\\
...
\subsection*{SWE5\_L1\_08}
\begin{description}
\item[ID] SWE5\_L1\_08
\item[Level] L1
\item[Criteria] SWE.5.BP7 --- Summarize and communicate results
\item[Identifies] Component/integration verification results not summarised/communicated
\item[Stakeholder] TE, SA, PM
\item[Question] How are software component-verification and integration-verification results summarised and communicated to affected parties?
\item[Options] ~\\
...
\subsection*{SWE5\_L2\_07}
\begin{description}
\item[ID] SWE5\_L2\_07
\item[Level] L2
\item[Criteria] GP 2.1.4 --- Adjust process performance (interface management between SD \& integration team)
\item[Identifies] Interface miscommunication between teams
\item[Stakeholder] TL, PM
\item[Question] How are interface changes coordinated between development and integration teams?
\item[Options] ~\\
...
\subsection*{SWE5\_L2\_09}
\begin{description}
\item[ID] SWE5\_L2\_09
\item[Level] L2
\item[Criteria] GP 2.2.2, GP 2.2.3 --- Storage and control of work products (component/integration verification reports)
\item[Identifies] Integration-verification reports not under configuration control
\item[Stakeholder] QA
\item[Question] How are component-verification and integration-verification reports stored, versioned, and controlled?
\item[Options] ~\\
...
\subsection*{SWE5\_L3\_10}
\begin{description}
\item[ID] SWE5\_L3\_10
\item[Level] L3
\item[Criteria] GP 3.1.1 --- Standard process (integration strategy)
\item[Identifies] No organisation-wide standard for integration approach
\item[Stakeholder] ASR, SA, PM
\item[Question] Does the organisation define a standard integration strategy (e.g., stepwise vs. continuous integration, environments to be used) that projects tailor from?
\item[Options] ~\\
...
\subsection*{SWE6\_L1\_01}
\begin{description}
\item[ID] SWE6\_L1\_01
\item[Level] L1
\item[Criteria] SWE.6.BP1 --- Specify verification measures vs. SW requirements
\item[Identifies] Test cases not derived from SW requirements
\item[Stakeholder] TE
\item[Question] How are software-verification measures derived from SW requirements?
\item[Options] ~\\
...
\subsection*{SWE6\_L1\_02}
\begin{description}
\item[ID] SWE6\_L1\_02
\item[Level] L1
\item[Criteria] SWE.6.BP2 --- Select verification measures incl. regression (release scope)
\item[Identifies] Regression policy weak at qualification
\item[Stakeholder] TE, PM
\item[Question] How are SWE.6 verification measures selected for the release, including regression?
\item[Options] ~\\
...
\subsection*{SWE6\_L1\_03}
\begin{description}
\item[ID] SWE6\_L1\_03
\item[Level] L1
\item[Criteria] SWE.6.BP3 --- Verify integrated SW \& record (qualification environment)
\item[Identifies] Wrong target / non-representative env.
\item[Stakeholder] TE, QA
\item[Question] What environment is used for SWE.6 qualification verification?
\item[Options] ~\\
...
\subsection*{SWE6\_L1\_04}
\begin{description}
\item[ID] SWE6\_L1\_04
\item[Level] L1
\item[Criteria] SWE.6.BP4 --- Consistency \& bidirectional traceability (SW reqs $\leftrightarrow$ test specs $\leftrightarrow$ results)
\item[Identifies] Final-stage traceability gap
\item[Stakeholder] QA, ASR
\item[Question] How is bidirectional traceability maintained between SW requirements, qualification test specs, and results?
\item[Options] ~\\
...
\subsection*{SWE6\_L1\_05}
\begin{description}
\item[ID] SWE6\_L1\_05
\item[Level] L1
\item[Criteria] SWE.6.BP5 --- Summarise \& communicate results
\item[Identifies] Defect leakage / no test summary
\item[Stakeholder] QA, PM
\item[Question] How are qualification-test results summarised and communicated to affected parties?
\item[Options] ~\\
...
\subsection*{SWE6\_L2\_06}
\begin{description}
\item[ID] SWE6\_L2\_06
\item[Level] L2
\item[Criteria] GP 2.2.2 --- Work-product requirements (defect management)
\item[Identifies] Defect process weak at qualification
\item[Stakeholder] QA, TE
\item[Question] How are qualification-test defects managed and triaged?
\item[Options] ~\\
...
\subsection*{SWE6\_L2\_08}
\begin{description}
\item[ID] SWE6\_L2\_08
\item[Level] L2
\item[Criteria] GP 2.1.1, GP 2.1.2, GP 2.1.3 --- Strategy, planning and resource readiness for SWE.6
\item[Identifies] Qualification-verification activity not planned/resourced ahead of release
\item[Stakeholder] PM, TE, TL
\item[Question] How is the SWE.6 qualification-verification activity planned and resourced ahead of a release (schedule, staffing, target-environment availability)?
\item[Options] ~\\
...
\subsection*{SWE6\_L3\_07}
\begin{description}
\item[ID] SWE6\_L3\_07
\item[Level] L3
\item[Criteria] GP 3.2.4 --- Monitor performance of defined process (information, not data)
\item[Identifies] No process-improvement feedback loop
\item[Stakeholder] ASR, QA, PM
\item[Question] How is information from SWE.6 fed back into the organisational standard process?
\item[Options] ~\\
...
\subsection*{SWE6\_L3\_09}
\begin{description}
\item[ID] SWE6\_L3\_09
\item[Level] L3
\item[Criteria] GP 3.1.1 --- Standard process (qualification-test strategy)
\item[Identifies] No organisation-wide standard qualification-test methodology
\item[Stakeholder] ASR, TE, PM
\item[Question] Does the organisation maintain a standard qualification-test methodology/template (e.g., technique-selection guideline, standard test-environment setup) that projects tailor from?
\item[Options] ~\\
