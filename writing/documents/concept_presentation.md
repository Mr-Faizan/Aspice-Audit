...
## Deviations & Evolution from the Original Concept

Comparing every claim in this presentation against what `Architecture.md`, `README.md`, and
`Implementation.md` show was actually built. Use this list directly for Ch.7.2 (Reflection on
Methodology) and/or Ch.8.3 (Limitations) — none of these are problems to hide; documenting a
deliberate, reasoned deviation is normal and expected in a methodology reflection.

1. **Question selection: batch vs. sequential.** Pitched: CMAB selects a **top-*k* set** of
   questions per round (Algorithm A step 3, Algorithm B step 2 — "select question set *Q*"),
   matching the supervisor papers' "5 questions per round" pattern. **Actual:** the
   implemented system selects **one question at a time**, rebuilding the 23-dimensional
   context vector after every single answer before selecting the next (`Implementation.md`
   §4, steps 3–4). This is arguably a *design improvement* (fresher context per decision
   point) but is a clear deviation from the pitched batch-selection algorithm.

2. **Weakness classification granularity: 3 categories vs. 4, and per-process vs.
   per-(process×level).** Pitched: "Good / Medium / Bad" (**3-category**) classification
   **per SWE process** (6 cells total: one per SWE.1–SWE.6). **Actual:** a **4-category**
   scheme (Compliant / Minor Weakness / Significant Gap / Critical Gap, at 0.25/0.50/0.75
   thresholds) applied **per (process × capability level) cell** — **18 cells** total (6
   processes × 3 levels), per `Architecture.md` §5. This is a substantially richer and more
   ASPICE-faithful design than what was originally pitched (it actually reflects ASPICE's own
   PA-per-capability-level structure) — worth explicitly claiming as a positive evolution in
   Ch.4.6/Ch.7.2, not just silently changing the number without comment.

3. **Database technology: undecided vs. committed.** Pitched: "MySQL **or** Postgres."
   **Actual:** PostgreSQL specifically, via SQLModel ORM with Alembic migrations — a concrete
   commitment the pitch left open.

4. **Backend framework: unspecified vs. FastAPI.** Pitched: "Backend: Python (business logic,
   evaluation controller)" — no framework named. **Actual:** FastAPI specifically, with a
   defined `app/api/routes/`, `app/services/`, `app/core/` structure (`backend/README.md`).

5. **Bandit algorithm: generic "CMAB/RL" vs. committed LinUCB.** Pitched: only ever says
   "Reinforcement Learning and Contextual Multi-Armed Bandits (CMAB)" generically — no
   specific algorithm named, and no mention of the specific citations [3]/[11]/[12] mapping to
   a chosen algorithm. **Actual:** a specific, deliberate commitment to **LinUCB with α = 1.0**
   (`README.md`), with an explicit rationale (determinism/reproducibility for thesis
   experiments) documented in `Architecture.md` §8 — a case where the pitch stayed
   deliberately open and the implementation phase made and justified a concrete algorithmic
   choice.

6. **Authentication: not mentioned vs. JWT.** Pitched implementation slides don't mention
   authentication/authorization at all. **Actual:** JWT-based auth is a core, documented part
   of the system (`README.md` Technology Stack table). Not a contradiction, just an
   unpitched-but-necessary addition — mention briefly in Ch.5 if discussing implementation
   completeness relative to the original plan.

7. **Docker: "optional" vs. core workflow.** Pitched: "Deployment (optional): Docker."
   **Actual:** Docker Compose (with `docker compose watch`) is the **primary, documented
   development workflow**, not an optional deployment afterthought (`README.md` Quick Start).

8. **Stakeholder roles: unspecified count vs. exactly seven, named.** Pitched: refers only to
   generic "stakeholder(s)" and "stakeholder role(s)" — no specific list or count is given
   anywhere in this presentation. **Actual:** exactly **seven, explicitly named** roles
   (Software Developer, Software Architect, Project Manager, QA Engineer, Test Engineer, Team
   Lead, ASPICE Assessor), matching the "seven key roles" framing from the 2024 supervisor
   paper's abstract. Worth an explicit sentence in Ch.3.6 or a footnote clarifying whether this
   exact list was adopted from the supervisor's own paper or derived independently during the
   implementation phase (this question was already flagged in
   `supervisor_research_paper_new_approach_early_detection_vulnerabilities.md`).

9. **Session/question budget: unspecified vs. 12 questions.** Pitched: no specific number of
   questions per session/release cycle is committed to anywhere in the presentation (only "a
   small number of well-targeted questions"). **Actual:** a specific `max_questions = 12`
   default per session, with documented rationale (`Architecture.md` §4: "67% coverage of the
   6×3=18 combinations... short enough for a real stakeholder to complete in a single
   sitting").

10. **SWE.6 naming: "Qualification Test" (v3.1) vs. "Software Verification" (v4.0).** Pitched
    presentation's own Figure 1 uses the older PAM v3.1 name for SWE.6. **Actual** codebase
    and `Architecture.md`/question bank consistently use PAM v4.0 naming ("Software
    Verification"). A minor terminology drift to reconcile, not a substantive one — but check
    it doesn't leak into your thesis text inconsistently.

11. **Timeline.** Pitched Gantt chart (Figure 7) planned final submission around **31 July**
    of the presentation year, with report writing concentrated in **July–August**. Given the
    session context in which this thesis writing work is actually happening, it's worth a
    frank paragraph in Ch.7.2 comparing the planned vs. actual schedule — do not silently let
    the thesis imply the original Gantt chart was met if it wasn't.

12. **"Content validation with supervisor" as a discrete, documented QA step.** Pitched as
    one of four named evaluation/testing activities (§7 above). Check whether this actually
    happened as a traceable event (meeting notes, a documented review round) — if it was
    informal/undocumented, that's a legitimate limitation to name alongside the
    already-flagged "no expert-validated weights" gap (cf. the 2025 supervisor paper's `.md`
    file, §III.C.1 callout).
